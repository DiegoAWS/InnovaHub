...
class MultiSelect2 {
    ...
    _handleClick(event) {
        ...
        if (this._state.opened) {
            const option = this._options.find(
                _option => _option.get() === event.target
            );
            ...
            //Remove Class to move Placeholder to Normal Position also on _unselectOption(event)
            if (this._selectedOptions().length == 0) {
                this._placeholder.removeClass(
                    "multi-select_placeholder_retracted"
                );
            }
        ...
        this._placeholder.addClass("multi-select_placeholder_retracted");
        ...
        if (
            "fa " + event.target.parentElement.classList[1] ===
            this._config.icon
        ) {
        ...
        if (this._config.multiple) {
            const options = this._config.value.map(_value => {
                ...
                const optionNode = this._options.find(
                    _option =>
                        _option.get().getAttribute("data-value") ===
    ...
    _unselectOption(event) {
        ...
        this._setValue(newValue, true, true);
          ...
          //Remove Class to move Placeholder to Normal Position also on _handleClick()
        if (this._selectedOptions().length == 0) {
            this._placeholder.removeClass(
                "multi-select_placeholder_retracted"
            );
    ...
    _nextUnselected(x) {
        ...
        while (
            x[this.currentFocus] &&
            x[this.currentFocus].classList.contains(
